<?php
/**
 * Gravity Flow Entry Detail Page
 *
 * @package     GravityFlow
...
/**
 * Class Gravity_Flow_Entry_Detail_Page
 */
class Gravity_Flow_Entry_Detail_Page {
	 ...
	 */
	public function entry_detail( $current_step = null, $args = array() ) {

		// In case fields need the GFEntryDetail class.
		require_once( GFCommon::get_base_path() . '/entry_detail.php' );

		$form_id      = absint( $this->form['id'] );
		$ajax         = false;
		$field_values = null;
		$entry        = $this->entry;
		$form         = $this->form;
		...
		<div class="wrap gf_entry_wrap gravityflow_workflow_wrap gravityflow_workflow_detail">
			...
			<?php
			$this->maybe_display_back_link( $args );
			$this->maybe_show_header( $args );

			$permission_granted = $check_view_entry_permissions ? $this->is_permission_granted( $current_step ) : true;
				...
				<div id="poststuff">
					<div id="post-body" class="metabox-holder <?php echo $classes; ?>">
						<div id="post-body-content">
							...
							if ( $current_step ) {
								$can_update = $this->can_update( $current_step );
								if ( $can_update ) {
							...
							if ( $instructions_step && $display_instructions ) {
								$this->maybe_show_instructions( $instructions_step );
							}

							$this->entry_detail_grid( $display_empty_fields, $editable_fields, $current_step );
							...
							if ( ! $sidebar ) {
								...
								$this->print_button( $show_timeline, $check_view_entry_permissions );
							}
							?>
						...
						<?php
						$this->maybe_show_timeline( $show_timeline );
						?>
					</div>
	 ...
	 */
	public function maybe_show_header( $args ) {
		$form = $this->form;
	 ...
	 */
	public function maybe_display_back_link( $args ) {
	 ...
	 * @return bool
	 */
	public function is_permission_granted( $current_step ) {
		global $current_user;
	 ...
	 * @return bool
	 */
	public function can_update( $current_step ) {
	 ...
	 * @param Gravity_Flow_Step $current_step         The step this entry is currently on.
	 */
	public function maybe_show_instructions( $current_step ) {
		...
		$instructions = $this->maybe_sanitize_instructions( $instructions );
	 ...
	 * @return string
	 */
	public function maybe_sanitize_instructions( $instructions ) {
	 ...
	 */
	public function print_button( $show_timeline, $check_view_entry_permissions ) {
		$entry = $this->entry;
	 ...
	 * @param bool  $show_timeline Indicates if the timeline should be displayed.
	 */
	public function maybe_show_timeline( $show_timeline ) {
		if ( ! $show_timeline ) {
			return;
		}
	...
	public function timeline() {
		$notes = $this->get_timeline_notes();
		$this->notes_grid( $notes );
	}
	 ...
	 * @return array
	 */
	public function get_timeline_notes() {
		return Gravity_Flow_Common::get_timeline_notes( $this->entry );
	}
	 ...
	 * @return string
	 */
	public function get_avatar( $user_id, $step ) {
		if ( $user_id ) {
	 ...
	 * @return string
	 */
	public function get_note_header( $display_name, $date_created ) {
		return sprintf(
	 ...
	 */
	public function notes_grid( $notes, $is_editable = false, $emails = null, $subject = '' ) {
		if ( empty( $notes ) ) {
			return;
		}
		...
		foreach ( $notes as $note ) {
			...
			echo sprintf(
				...
				esc_attr( $note->id ),
				esc_attr( $step_type ),
				$this->get_avatar( $user_id, $step ),
				$this->get_note_body( $note, $display_name )
			);
		}
	}
	 ...
	 */
	public function entry_detail_grid( $allow_display_empty_fields = false, $editable_fields = array(), $current_step = null ) {
		$form    = $this->form;
		$entry   = $this->entry;
		...
		<table cellspacing="0" class="widefat fixed entry-detail-view <?php echo esc_attr( $step_class ) ?>">
			...
			if ( empty( $editable_fields ) ) {
				...
				<tbody>
				<?php
				$this->fields( $display_empty_fields, $current_step, 'table' );
				?>
				</tbody>
				<?php
			} else {
				$this->entry_editor( $current_step, $display_empty_fields );
			}
	 ...
	 */
	public function entry_editor( $current_step, $display_empty_fields ) {
		$form  = $this->form;
		$entry = $this->entry;
		?>
		<tbody>
		...
		<?php
		$this->maybe_show_products_summary( $current_step );
		?>
		</tbody>
		<?php
	}
	 ...
	 * @param Gravity_Flow_Step $current_step The step this entry is currently on.
	 */
	private function maybe_show_products_summary( $current_step ) {
		$form            = $this->form;
		$entry           = $this->entry;
		...
		<tr>
			<td colspan="2" class="entry-view-field-value lastrow">
				<?php $this->products_summary( $products ) ?>
			</td>
		</tr>
		<?php
	 ...
	 */
	public function fields( $display_empty_fields, $current_step, $format ) {
		$form               = $this->form;
		$entry              = $this->entry;
		$form_id            = absint( $form['id'] );
		$count              = 0;
		...
		foreach ( $form['fields'] as &$field ) {
			...
			$display_field = $current_step && $is_assignee ? $this->is_display_field( $field, $current_step, $is_product_field ) : $this->is_display_field( $field, $display_fields_step, $is_product_field );
			...
			switch ( RGFormsModel::get_input_type( $field ) ) {
				case 'section' :
					...
					if ( ! $this->is_section_empty( $field, $current_step, $display_empty_fields ) ) {
						$count ++;
				...
				default :
					...
					$display_value = $this->get_display_value( $value, $field );
					...
					if ( $display_empty_fields || ! empty( $display_value ) || $display_value === '0' ) {
						...
						$content = '
                                <tr>
                                    <td colspan="2" class="entry-view-field-name">' . esc_html( $this->get_label( $field ) ) . '</td>
                                </tr>
                                <tr>
	 ...
	 * @return bool
	 */
	private function is_section_empty( $section_field, $current_step, $display_empty_fields ) {
		$form      = $this->form;
		$entry     = $this->entry;
		$cache_key = "Gravity_Flow_Entry_Detail_Page::is_section_empty_{$form['id']}_{$section_field->id}_{$display_empty_fields}";
		...
		foreach ( $section_fields as $field ) {
			...
			$is_product_field = GFCommon::is_product_field( $field->type );
			$display_field    = $this->is_display_field( $field, $current_step, $is_product_field );
	 ...
	 * @return bool
	 */
	private function is_display_field( $field, $current_step, $is_product_field ) {
		return Gravity_Flow_Common::is_display_field( $field, $current_step, $this->form, $this->entry, $is_product_field );
	}
	 ...
	 * @return string
	 */
	public function get_display_value( $value, $field ) {
		$entry = $this->entry;
		$form  = $this->form;
	 ...
	 */
	public function products_summary( $products ) {
		$form    = $this->form;
		$entry   = $this->entry;
